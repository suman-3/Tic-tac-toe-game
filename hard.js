...
const makeComputerMove = () => {
  ...
  if (winningMove !== null) {
    ...
    boxtexts[winningMove].style.color = "violet"; // Set color to blue
// for O's turn
...
boxtexts[blockingMove].style.color = "violet"; // Set color to blue for O's turn
audioTurn0.play(); // Play audioTurn0 for O's turn
turn = changeTurn();
checkWin();
if (!isgameover) {
...
boxtexts[selectedBox].style.color = "violet"; // Set color to blue for O's turn
audioTurn0.play(); // Play audioTurn0 for O's turn
turn = changeTurn();
checkWin();
...
if (boxtext.innerText === "" && !isgameover) {
  ...
  if (turn === "X") {
    boxtext.style.color = "black"; // Set color to gray for X's turn
    audioTurn.play(); // Play audioTurn for X's turn
  } else {
    boxtext.style.color = "violet"; // Set color to violet for O's turn
    audioTurn0.play(); // Play audioTurn0 for O's turn
  }
  turn = changeTurn();
  checkWin();
  if (!isgameover) {
    ...
    if (turn === "O") {
        setTimeout(makeComputerMove, 800); // Delay computer's move by 1 second
    }
  }
}
});
...
reset.addEventListener("click", () => {
    ...
    restAudio.play(); // Play the reset audio
